...
typeorm_seeding_1.define(Post_1.Post, function (faker) {
    var title = faker.random.word();
    var content = '하나에 멀리 별빛이 하나 그리워 풀이 거외다. 속의 우는 밤을 시와 책상을 벌써 새워 버리었습니다. 이름자 이름자를 시인의 릴케 나의 된 계십니다. 이웃 옥 않은 이국 봅니다. 토끼, 하나의 나의 노루, 북간도에 헤는 아스라히 까닭입니다. 이름자를 묻힌 나의 이네들은 이 이름과, 슬퍼하는 어머니, 봄이 듯합니다. 같이 가슴속에 별이 오는 이 멀리 계절이 당신은 봅니다. 걱정도 파란 어머니, 봅니다. 둘 아침이 내린 소녀들의 토끼, 아스라히 듯합니다. 남은 가난한 이름을 당신은 애기 동경과 버리었습니다.<br>' +
    ...
    post.sort = sort;
    post.tags = tags;
    post.user = typeorm_seeding_1.factory(User_1.User)();
    return post;
});
